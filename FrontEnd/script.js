...
function generarLienzo ()  {
    ...
    try{
        buscaRef = lista_referenciaJS.indexOf(ref);
    }catch (er){
        buscaRef = -1;
    }
    

    if(buscaRef != -1 || ref.length < 3){
...
function volver () {//Esta función permite cambiar el tamaño del lienzo para poder volver a tomar otra medición
    location.reload();
}
...
class CalculoCoordenadas {
    ...
    calculoCoordenadas () {
        let anchura = parseInt(document.getElementById('width').value);//Estas dos variables sirven para obtener la altura y la anchura del recipiente, y así los poder utilizar dimensiones personalizdas
        let altura = parseInt(document.getElementById('height').value);
...
function escribirCoordenadas (numPts) {
    addEventListener('keypress', () => {
        ...
        if (tecla == 'Enter') {
            ...
            let datos = {
                'nPuntos': numPts,
                'movimientos1': cadenaDeTxtBDD,
                'movimientos2': cadenaDeTxtBDD2,
                'movimientos3': cadenaDeTxtBDD3,
                'movimientos4': cadenaDeTxtBDD4,
                'altura': altura,
                'anchura': anchura,
                'referencia': referencia
            // nPuntos | movimientos | altura | anchura | referencia
            };
    ...
    if (document.getElementById('inferior').style.display == 'none' && !comprobarDistancia) {
        ...
        switch (numPts) { //Con este switch se comprueba el número de puntos seleccionados para rastrear para que no salgan las coordenadas de puntos que no se están rastreando
            ...
            case 3:
                ...
                break;
            case 4:
                document.getElementById('botonCoordenadas').style.display = 'block' //Como sse devuelven los datos en forma de matriz se van escibiendo cada uno en sus respectivos apartados
                document.getElementById('Instrucciones').innerHTML = `
                <div id="izda">
                <ul>
                    <li class="negrita">Coordenadas:</li>
